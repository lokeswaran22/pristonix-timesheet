...
// Delete single audit log entry by ID
app.delete('/api/audit/history/:id', async (req, res) => {
    const { id } = req.params;

    try {
        const result = await query('DELETE FROM activity_history WHERE id = $1', [id]);
...
// Bulk delete audit logs (with filters)
app.delete('/api/audit/history', async (req, res) => {
    ...
    try {
        // Clear all logs
        if (clearAll === 'true') {
            const result = await query('DELETE FROM activity_history');
            return res.json({
                success: true,
        ...
        // Build filtered delete query
        let deleteQuery = 'DELETE FROM activity_history WHERE 1=1';
        const params = [];
        let paramCount = 1;

        if (startDate) {
            deleteQuery += ` AND date_key >= $${paramCount}`;
        ...
        if (name) {
            deleteQuery += ` AND user_name ILIKE $${paramCount}`;
            params.push(`%${name}%`);
            paramCount++;
        }

        // Require at least one filter if not clearing all
        if (params.length === 0) {
            return res.status(400).json({
                error: 'Please provide filters (startDate, endDate, or name) or use clearAll=true'
            });
        }
        ...
        res.json({
            success: true,
            message: `Deleted ${result.rowCount} audit log(s)`,
            deletedCount: result.rowCount
        });
    } catch (err) {
