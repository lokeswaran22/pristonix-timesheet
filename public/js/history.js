class HistoryManager {
    ...
    renderTable(logs) {
        ...
        logs.forEach(log => {
            ...
            // User Name - check both camelCase and snake_case
            const userName = log.user_name || log.userName || `User #${log.user_id}`;

            // Action By - check both camelCase and snake_case
            const doneBy = log.action_by_name || log.actionByName || (log.action_by === log.user_id ? 'Self' : `User #${log.action_by}`);
    ...
    getFilteredLogs() {
        ...
        return this.rawLogs.filter(log => {
            ...
            const user = log.user_name || log.userName || '';
            ...
            <body>
                ...
                <table>
                    ...
                    <tbody>
                        ${filtered.map(log => {
            const data = log.new_data || log.old_data || {};
            const user = log.user_name || log.userName || log.user_id;
            const by = log.action_by_name || log.actionByName || (log.action_by === log.user_id ? 'Self' : log.action_by);
