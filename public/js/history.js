class HistoryManager {
        ...
        logs.forEach(log => {
            ...
            row.innerHTML = `
                 ...
                 <td style="text-align: center;">
                    <button class="delete-log-btn" data-log-id="${log.id}" style="
        ...
        if (historyBody) {
            historyBody.addEventListener('click', async (e) => {
                if (e.target.closest('.delete-log-btn')) {
                    ...
                    try {
                        const res = await fetch(`/api/audit/history/${logId}`, {
                            method: 'DELETE'
                        });
    ...
    generateComplianceReport() {
        ...
        printWindow.document.write(`
            ...
            let by = log.action_by_name || log.actionByName || log.action_by;

            // Show "Self" if user edited their own data
            if (log.action_by === log.user_id || by === user) {
                by = 'Self';
            }
