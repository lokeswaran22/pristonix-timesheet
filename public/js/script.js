...
class ActivityTracker {
    ...
    async loadLogs(dateKey = null) {
        ...
        // Use the new audit history endpoint instead of legacy activity-log
        let url = '/api/audit/history?limit=50';
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Transform audit history format to match expected format
                const transformedLogs = logs.map(log => ({
                    employeeName: log.user_name || log.userName || `User #${log.user_id}`,
                    activityType: (log.new_data || log.old_data || {}).type || 'Unknown',
                    description: (log.new_data || log.old_data || {}).description || '',
                    timeSlot: log.time_slot,
                    action: log.action_type,
                    editedBy: log.action_by_name || log.actionByName || 'System',
                    timestamp: log.action_timestamp,
                    dateKey: log.date_key
                }));
                this.render(transformedLogs);
            }
        } catch (e) {
